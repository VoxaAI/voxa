...
const AlexaError = require('./Errors');
const capitalize = require('capitalize');
const createServer = require('./create-server');

const UnknownRequestType = AlexaError.UnkownRequestType;
const ErrorOnSessionEnded = AlexaError.ErrorOnSessionEnded;
      ...
      switch (alexaEvent.request.type) {
        ...
        case 'SessionEndedRequest': {
          ...
          return Promise.mapSeries(this.getOnRequestStartedHandlers(), fn => fn(alexaEvent, reply))
            .then(() => {
              if (alexaEvent.request.type === 'SessionEndedRequest' && _.get(alexaEvent, 'request.reason') === 'ERROR') {
                throw new ErrorOnSessionEnded(_.get(alexaEvent, 'request.error'));
              }
