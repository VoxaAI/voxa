...
class AlexaSkill {
  ...
  execute(event) {
    ...
    return Promise.try(() => {
      ...
      if (this.config.appIds && !_.includes(this.config.appIds, event.session.application.applicationId)) {
        debug(`The applicationIds don't match: "${event.session.application.applicationId}"  and  "${this.config.appIds}"`);
