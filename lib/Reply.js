...
class Reply {
  ...
  append(msg) {
    ...
    this.msg.directives = this.msg.directives.concat(cannonicalizeDirectives(this.alexaEvent, msg.directives));
    return this;
  }
  ...
  appendReply(reply) {
    ...
    this.msg.directives = this.msg.directives.concat(cannonicalizeDirectives(this.alexaEvent, this.msg.directives));
...
function cannonicalizeDirectives(alexaEvent, directives) {
  if (!directives) return [];
  if (!_.isArray(directives)) directives = [directives];
  return directives
  .filter((directive) => {
    const hasDisplayInterface = _.get(alexaEvent, 'context.System.device.supportedInterfaces.Display');
    const directivesType = directive.type;
    if (!hasDisplayInterface && _.includes(['Display.RenderTemplate', 'Hint'], directivesType)) return false;
    return true;
  })
