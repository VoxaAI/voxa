...
class Reply {
  ...
  appendReply(reply) {
    ...
    this.msg.yield = reply.msg.yield || this.msg.yield;
    this.msg.hasAnAsk = reply.msg.hasAnAsk || this.msg.hasAnAsk;

    reply.msg.supportDisplayInterface = reply.msg.supportDisplayInterface ||
      hasSupportForDisplay(this.alexaEvent);

    this.msg.directives = reply.msg.directives
    .concat(cannonicalizeDirectives(this.alexaEvent, reply.msg));

    this.msg.terminate = !(reply.msg.hasAnAsk || this.hasDirective(/^Dialog\./)) && (!!reply.msg.terminate || this.msg.terminate);
  ...
  toJSON() {
    ...
    if (this.canFulfillIntent) {
      ...
      const hasVideoDirective = this.hasDirective('VideoApp.Launch');
      const hasGameEngineDirective = this.hasDirective('GameEngine.StartInputHandler');

      if (!hasVideoDirective && !hasGameEngineDirective) {
