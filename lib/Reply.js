...
class Reply {
  ...
  append(msg) {
    ...
    if (msg.directives) {
      if (_.isArray(msg.directives)) {
        this.msg.directives = _.concat(this.msg.directives, msg.directives);
      } else {
        this.msg.directives.push(msg.directives);
      }
    }
    return this;
  }
  ...
  appendReply(reply) {
    ...
    if (reply.msg.directives) {
      this.msg.directives = _.concat(this.msg.directives, reply.msg.directives);
    }
  ...
  static buildSpeechletResponse(options) {
    ...
    if (options.directives.length) {
      alexaResponse.directives = _(options.directives)
        .map((directive) => {
          if (directive.playBehavior) {
            return {
              type: directive.type,
