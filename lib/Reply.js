...
class Reply {
  ...
  toJSON() {
    ...
    if (directives) {
      if (directives.playBehavior) {
        alexaResponse.directives = [
          {
            type: directives.type,
            playBehavior: directives.playBehavior,
            audioItem: {
              stream: {
                token: directives.token,
                url: directives.url,
                offsetInMilliseconds: directives.offsetInMilliseconds,
              },
            },
          },
        ];
      } else if (directives.type) {
        alexaResponse.directives = [
          {
            type: directives.type,
          },
    ...
    if (this.session && this.session.attributes) {
...
function cannonicalizeDirective(directive) {
  // Legacy support. We used to support this slightly altered from spec version of athe play directive.
  // We'll check if we think that's still happening and transform to the full Alexa directive
  if (directive.playBehavior && (directive.token || directives.url)) {
    return {
      type: directive.type,
