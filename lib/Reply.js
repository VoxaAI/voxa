...
function cannonicalizeDirective(directive) {
  ...
  if (directive.playBehavior && (directive.token || directive.url)) {
    ...
    if (directive.expectedPreviousToken) {
      _.set(directiveToSend, 'audioItem.stream.expectedPreviousToken', directive.expectedPreviousToken);
    }
    return directiveToSend;
  }
