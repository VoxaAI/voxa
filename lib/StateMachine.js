...
class StateMachine {
      ...
      return Promise
        ...
        .then((onUnhandledStateTransition) => {
          if (!onUnhandledStateTransition) {
            throw new errors.UnhandledState(voxaEvent, onUnhandledStateTransition, this.currentState.name);
          }
  ...
  onAfterStateChanged(voxaEvent, reply, transition) {
    ...
    return Promise.mapSeries(this.onAfterStateChangeCallbacks, fn => fn(voxaEvent, reply, transition))
      .then(() => transition);
  }
  ...
  runCurrentState(voxaEvent) {
    if (_.get(this.currentState, ['enter', voxaEvent.intent.name])) {
      debug(`Running ${this.currentState.name} enter function for ${voxaEvent.intent.name}`);
      return Promise.try(() => this.currentState.enter[voxaEvent.intent.name](voxaEvent));
    }

    if (_.get(this.currentState, 'enter.entry')) {
      debug(`Running ${this.currentState.name} enter function entry`);
      return Promise.try(() => this.currentState.enter.entry(voxaEvent));
    }

    debug(`Running simpleTransition for ${this.currentState.name}`);
    const fromState = this.currentState;
    const to = _.get(fromState, ['to', voxaEvent.intent.name]);
    return simpleTransition.call(this, this.currentState, to);
    ...
    function simpleTransition(state, dest) {
      ...
      const destObj = this.states[dest];
      if (!destObj) {
        throw new errors.UnknownState(dest);
      }
