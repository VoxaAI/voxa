...
class StateMachine {
  ...
  checkOnUnhandledState(alexaEvent, transition) {
    const currentState = this.currentState.name;
    if (!transition) {
      debug('Running onUnhandledStateCallbacks');
      return Promise.mapSeries(this.onUnhandledStateCallbacks, fn => fn(alexaEvent, currentState))
        .then(_.last)
        .then((onUnhandledStateTransition) => {
          if (!onUnhandledStateTransition) {
            throw new errors.UnhandledState(alexaEvent, onUnhandledStateTransition, currentState);
          }
