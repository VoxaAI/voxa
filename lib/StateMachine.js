...
class StateMachine {
  ...
  transition(request, reply) {
    ...
    function runTransition() {
      return Promise.mapSeries(this.onBeforeStateChangedCallbacks, fn => fn(request, reply, this.currentState))
        .then(() => this.runCurrentState(request))
        .then((transition) => {
          if (!transition && this.currentState.name !== 'entry') {
            // If no response try falling back to entry
            debug(`No reply for ${request.request.intent.name} in [${this.currentState.name}]. Trying [entry].`);
        ...
        .then((transition) => {
          if (!transition) {
            ...
            return Promise.mapSeries(this.onUnhandledStateCallbacks, fn => fn(request, this.currentState.name))
              .then(_.last);
          }
        ...
        .then((transition) => {
          ...
          return Promise.mapSeries(this.onAfterStateChangeCallbacks, fn => fn(request, reply, transition))
            .then(() => transition);
        })
        .then((transition) => {
          let to;
          ...
          if (reply.isYielding() || !transition.to || transition.to.isTerminal) {
            const result = { to };
            if (_.isString(transition.reply)) {
  ...
  runCurrentState(request) {
    ...
    const destName = fromState.to[request.request.intent.name];
