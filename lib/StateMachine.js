      ...
      return Promise.mapSeries(this.onBeforeStateChangedCallbacks, fn => fn(request, reply))
        ...
        .then((transition) => {
          ...
          if (transition && !transition.to) {
            return _.merge(transition, { to: 'die' });
          }
        ...
        .then((transition) => {
          ...
          return Promise.mapSeries(this.onAfterStateChangeCallbacks,
            fn => fn(request, reply, transition))
            .then(() => transition);
        })
        .then((transition) => {
          debug(`${this.currentState.name} transition resulted in %s`, JSON.stringify(transition));
          let to;
          if (!transition) throw new errors.BadResponse(transition, this.currentState.name);
          if (transition.to) {
