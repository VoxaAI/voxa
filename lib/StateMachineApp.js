...
class StateMachineApp extends VoxaApp {
  constructor(config) {
    ...
    this.renderer = new this.config.RenderClass(this.config, this.i18nextPromise);
    ...
    this.onAfterStateChanged((voxaEvent, reply, transition) => {
      if (transition && transition.reply instanceof Reply) {
        reply.append(transition.reply);
        return transition;
      }

      return this.renderer.render(voxaEvent, reply, transition)
        .then((message) => {
          if (transition.directives) {
            reply.append(transition);
          }
  ...
  transformRequest(voxaEvent) {
    return this.i18nextPromise.then(() => this.config.Model.fromEvent(voxaEvent))
      .then((model) => {
        voxaEvent.model = model;
        voxaEvent.t = i18next.getFixedT(voxaEvent.request.locale);
