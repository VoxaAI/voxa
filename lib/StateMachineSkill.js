...
const CustomerContact = require('./apis/CustomerContact');
const DeviceAddress = require('./apis/DeviceAddress');
const DeviceSettings = require('./apis/DeviceSettings');
const InSkillPurchase = require('./apis/InSkillPurchase');
const Lists = require('./apis/Lists');
const LWA = require('./apis/LWA');
...
class StateMachineSkill extends AlexaSkill {
  constructor(config) {
    ...
    const requestToTurnIntoIntent = [
      'onLaunchRequest',
      'onConnections.Response',
      'onDisplay.ElementSelected',
      ...
      'onGameEngine.InputHandlerEvent',
    ];

    function requestNameToIntent(requestToConvert) {
      return _.chain(requestToConvert)
        .split('')
        .drop(2)
        .join('')
        .replace('.', '')
    ...
    _.each(requestToTurnIntoIntent, (requestToConvert) => {
      this[requestToConvert]((alexaEvent, reply) => {
        let intent;

        if (requestToConvert === 'onLaunchRequest') {
          intent = this.config.openIntent || 'LaunchIntent';
        } else {
          intent = requestNameToIntent(requestToConvert);
        }
  ...
  transformRequest(alexaEvent) {
    alexaEvent.customerContact = new CustomerContact(alexaEvent);
    alexaEvent.deviceAddress = new DeviceAddress(alexaEvent);
    alexaEvent.deviceSettings = new DeviceSettings(alexaEvent);
    alexaEvent.isp = new InSkillPurchase(alexaEvent);
    alexaEvent.lists = new Lists(alexaEvent);
    alexaEvent.lwa = new LWA(alexaEvent);

    return Promise.try(() => this.config.Model.fromEvent(alexaEvent))
      .then((model) => {
        if (alexaEvent.session.attributes.state === 'die') {
          alexaEvent.session.attributes.state = 'entry';
        }
