...
const VoxaSkill = require('./VoxaSkill');
...
class StateMachineSkill extends VoxaSkill {
  constructor(config) {
    super(config);
    this.states = {};
    ...
    this.onLaunchRequest((voxaEvent, reply) => {
      const intent = this.config.openIntent || 'LaunchIntent';
      _.set(voxaEvent, 'intent.name', intent);
      _.set(voxaEvent, 'intent.slots', {});

      return this.requestHandlers.IntentRequest(voxaEvent, reply);
    }, true);
