...
class VoxaApp {
    constructor(config) {
        ...
        this.onAfterStateChanged((voxaEvent, reply, transition) => __awaiter(this, void 0, void 0, function* () {
            const result = yield Promise.all(_.map((transition), (value, key) => __awaiter(this, void 0, void 0, function* () {
                const directiveHandler = _.find(reply.directiveHandlers, { key });
                if (!directiveHandler) {
                    return;
                }
                return yield directiveHandler.handler(value)(reply, voxaEvent);
            })));
            return yield transition;
        }));
        this.onBeforeReplySent((voxaEvent, reply, transition) => __awaiter(this, void 0, void 0, function* () {
    ...
    handleOnSessionEnded(voxaEvent, reply) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const replies = yield bluebird.mapSeries(this.getOnSessionEndedHandlers(), (fn) => fn(voxaEvent, reply));
            if (replies.length) {
    ...
    handleErrors(event, error, ReplyClass) {
        return __awaiter(this, void 0, void 0, function* () {
            const reply = yield bluebird.reduce(this.getOnErrorHandlers(), (reply, errorHandler) => {
                if (reply) {
                    return reply;
                }
            ...
            try {
                ...
                switch (voxaEvent.request.type) {
                    ...
                    case 'SessionEndedRequest': {
                        ...
                        const result = yield bluebird.mapSeries(this.getOnRequestStartedHandlers(), (fn) => fn(voxaEvent, voxaReply));
                        ...
                        yield bluebird.mapSeries(this.getOnSessionStartedHandlers(), (fn) => fn(voxaEvent, voxaReply));
                        // Route the request to the proper handler which may have been overriden.
                        return yield requestHandler(voxaEvent, voxaReply);
                    }
                    default: {
                        return yield requestHandler(voxaEvent, voxaReply);
                    }
    ...
    registerRequestHandler(requestType) {
        ...
        this.requestHandlers[requestType] = (voxaEvent, reply) => __awaiter(this, void 0, void 0, function* () {
            log(eventName);
            const capitalizedEventName = _.upperFirst(_.camelCase(eventName));
            const result = yield bluebird.mapSeries(this[`get${capitalizedEventName}Handlers`](), (fn) => fn.call(this, voxaEvent, reply));
    ...
    runStateMachine(voxaEvent, reply) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transition = yield stateMachine.runTransition(voxaEvent, reply);
            if (!_.isString(transition.to) && transition.to.isTerminal) {
                yield this.handleOnSessionEnded(voxaEvent, reply);
            }
            const onBeforeReplyHandlers = this.getOnBeforeReplySentHandlers();
            log('Running onBeforeReplySent');
            yield bluebird.mapSeries(onBeforeReplyHandlers, (fn) => fn(voxaEvent, reply, transition));
            return reply;
        });
    }
