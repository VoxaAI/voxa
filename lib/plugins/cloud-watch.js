...
function register(skill, cloudwatch, config) {
  ...
  skill.onStateMachineError(voxaEvent => Promise.try(() => {
    ...
    return putMetricDataAsync(onErrorMetric)
      .then((data) => {
        ...
        _.set(voxaEvent, 'cloudwatch.errorReported', true);
  ...
  skill.onError(voxaEvent => Promise.try(() => {
    const errorReported = _.get(voxaEvent, 'cloudwatch.errorReported');
