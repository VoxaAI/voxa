  ...
  renderMessage(msg, alexaEvent) {
    function deepSearchStatement(statement, alexaEvent) {
      if (_.isObject(statement) && !_.isArray(statement)) {
        return Promise.try(() => {
          ...
          .map(item => {
            ...
            if (isAnOpenResponse &&  _.isArray(value)) return [key, deepSearchStatement.call(this, _.sample(value), alexaEvent)];
            return [key, deepSearchStatement.call(this, value, alexaEvent)];
          })
          .flattenDeep()
          .value();

          return Promise.all(objPromises).then(result => _.chain(result).chunk(2).fromPairs().value())
        });
      }
      ...
      if (_.isArray(statement)) {
        return Promise.all(statement.map(statementItem => deepSearchStatement.call(this, statementItem, alexaEvent)));
      }

      return Promise.resolve(statement);
    }

    return deepSearchStatement.call(this, msg, alexaEvent);
  }

  renderStatement(statement, alexaEvent) {
    const tokenRegx = /{([\s\S]+?)}/g;
    _.templateSettings.interpolate = tokenRegx;
    const tokenKeys = _.uniq(statement.match(tokenRegx) || []).map(str => str.substring(1, str.length - 1));
