...
var CortanaReply = /** @class */ (function (_super) {
    ...
    CortanaReply.prototype.toJSON = function () {
        ...
        var text = this.response.statements.join("\n");
        ...
        var attachments = _(this.response.directives)
            .filter(function (directive) { return !!directive.attachment; })
            .map("attachment.data")
            .value();
        var suggestedActions = _(this.response.directives)
            .filter(function (directive) { return !!directive.suggestedActions; })
